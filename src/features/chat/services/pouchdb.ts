    ...
    async getMessages(child_id?: string, limit: number = 30, skip: number = 0): Promise<LocalMessage[]> {
        ...
        try {
            ...
            const result = await this.db.find({
                selector: selector,
                // Remove sort from query to avoid index mismatch errors
                // limit/skip still work but sort is safer in memory for PouchDB
            });

            const docs = result.docs as LocalMessage[];

            // Sort in memory: newest first (descending timestamp)
            docs.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

            // Apply manual pagination if needed (though find does it, but sorting changes the set)
